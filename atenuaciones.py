...
def determinar_danio_dominante(row):
    ...
    if los > power and los > 0:
        return "FIBRA"
    elif power > los and power > 0:
        return "ENERGIA"
    elif los == 0 and power == 0:
        return "SIN_CORTE"
    else:
        return "MIXTO"
...
def clasificar_prioridad(row):
    if row['puerto_muerto']:
        return "ALTA"
    if row['score_prioridad'] >= 100:
        return "ALTA"
    elif row['score_prioridad'] >= 50:
        return "MEDIA"
    else:
        return "BAJA"
...
def analizar_senal(df, olt, columna_signal):
    df_olt = df[df['OLT'] == olt]

    resumen = (
        df_olt
        .groupby(['OLT', 'Board', 'Port'])
        .agg(
            promedio_signal=(columna_signal, 'mean'),
            total_onus=(columna_signal, 'count'),
    ...
    if columna_signal == "Signal 1310":
        resumen['estado'] = resumen['promedio_signal'].apply(clasificar_estado_1310)
    else:
        resumen['estado'] = resumen['promedio_signal'].apply(clasificar_estado_1490)

    # Solo arpónes problemáticos
    resumen = resumen[resumen['estado'] != "Normal"]

    # Métricas operativas
    resumen['peso_estado'] = resumen['estado'].map(PESOS_ESTADO)
    resumen['score_prioridad'] = resumen['total_onus'] * resumen['peso_estado']
    resumen['porcentaje_online'] = (
        (resumen['onus_online'] / resumen['total_onus']) * 100
    ).round(1)

    resumen['puerto_muerto'] = resumen['onus_online'] == 0
    resumen['senal'] = columna_signal.replace("Signal ", "")

    # Ordenar por criticidad real
...
def generar_agenda_general(df):
    ...
    # Prioridad (ya definida antes)
    agenda_general['prioridad'] = agenda_general.apply(
        clasificar_prioridad, axis=1
    )

    # 🆕 Daño dominante
    agenda_general['danio_dominante'] = agenda_general.apply(
        determinar_danio_dominante, axis=1
    )

    columnas = [
        'prioridad', 'danio_dominante', 'score_prioridad',
        'OLT', 'Board', 'Port', 'senal',
        'estado', 'promedio_signal',
        'total_onus',
        'onus_online', 'onus_offline', 'onus_los', 'onus_power_fail',
        'porcentaje_online', 'puerto_muerto',
        'address'
    ]

    agenda_general = agenda_general[columnas].sort_values(
        by=['prioridad', 'score_prioridad'],
...
def auditoria_atenuacion_csv(ruta_csv, salida_excel="auditoria_atenuacion.xlsx"):
    ...
    if "AGENDA_GENERAL" in wb.sheetnames:
        ws = wb["AGENDA_GENERAL"]
        colorear_prioridad(ws)
    wb.save(salida_excel)

    print(f"✔ Auditoría generada con prioridades: {salida_excel}")

auditoria_atenuacion_csv(
    ruta_csv="olt.csv",
    salida_excel="auditoria_olt_completa.xlsx"
)
